...
from drom.torch_utils.torch_utils import dict_to_device, DEFAULT_TENSOR_ARGS, to_numpy
...
def save_models_to_disk(models_prefix_l, epoch, total_steps, ckpt_dir=None):
    for model, prefix in models_prefix_l:
        if model is not None:
            save_model_to_disk(model, epoch, total_steps, ckpt_dir, prefix=f'{prefix}_')
            for submodule_key, submodule_value in model.submodules.items():
                save_model_to_disk(submodule_value, epoch, total_steps, ckpt_dir,
                                   prefix=f'{prefix}_{submodule_key}_')


def save_model_to_disk(model, epoch, total_steps, ckpt_dir=None, prefix='model_'):
    ...
    torch.save(model.state_dict(), os.path.join(ckpt_dir, f'{prefix}current_state_dict.pth'))
    torch.save(model.state_dict(), os.path.join(ckpt_dir, f'{prefix}epoch_{epoch:04d}_iter_{total_steps:06d}_state_dict.pth'))
    torch.save(model, os.path.join(ckpt_dir, f'{prefix}current.pth'))
    torch.save(model, os.path.join(ckpt_dir, f'{prefix}epoch_{epoch:04d}_iter_{total_steps:06d}.pth'))


def save_losses_to_disk(train_losses, val_losses, ckpt_dir=None):
    np.save(os.path.join(ckpt_dir, f'train_losses.npy'), train_losses)
    np.save(os.path.join(ckpt_dir, f'val_losses.npy'), val_losses)
...
def train(model=None, train_dataloader=None, epochs=None, lr=None, steps_til_summary=None,
          log_dir=None, ckpt_dir=None, video_dir=None, time_dir=None,
          loss_fn=None,
          train_subset=None,
    ...
    save_models_to_disk([(model, 'model'), (ema_model, 'ema_model')], 0, 0, ckpt_dir)
        ...
        for epoch in range(epochs):
            ...
            for step, train_batch_dict in enumerate(train_dataloader):
                ...
                with TimerCUDA() as t_training_loss:
                    ...
                    # print("step: " + str(step))
                    # print("traj_normalized: " + str(train_batch_dict["traj_normalized"].size()))
                    # print("task_normalized: " + str(train_batch_dict["task_normalized"].size()))
                ...
                if train_steps_current % steps_til_summary == 0:
                    # TRAINING
                    print(f"\n\n\n-------------------Summary----------------------")
                    print(f"Current training step: {train_steps_current}")
                    print(f"Training loss {train_losses["diffusion_loss"].item()}")
                    ...
                    if val_dataloader is not None:
                        with TimerCUDA() as t_validation_loss:
                            ...
                            for step_val, batch_dict_val in enumerate(val_dataloader):
                                ...
                                # batch_size = batch_dict_val["traj_normalized"].size(0)
                                # print(f"Batch number {step_val} with batch size of {batch_size}")
                                val_loss, val_loss_info = loss_fn(model, batch_dict_val, val_subset.dataset, step=train_steps_current)
                                for name, value in val_loss.items():
                                    single_loss = to_numpy(value, dtype=np.float64)
                                    # print(f"{name} = {single_loss}")
                            ...
                            for loss_name, loss in val_losses.items():
                                ...
                                validation_losses[f'{loss_name}'] = single_loss


                        # print(f"Computation time for validation_loss: {t_validation_loss.elapsed:.4f} sec")
                        print(f"Validation loss {validation_losses["diffusion_loss"]}")

                        validation_losses_log = validation_losses
                        validation_losses_l.append((train_steps_current, validation_losses_log))

                    logs = {
                        "Training Loss": train_losses["diffusion_loss"].item(),
                        "Validation Loss": validation_losses["diffusion_loss"]
                    }
                    wandb.log({**logs}, step=train_steps_current)
                    print("\n\n")
                # set model to training mode
                model.train()
                ...
                # print("Optimizing train loss batch")
                ...
                # if train_steps_current % steps_til_summary == 0:
                    # print(f"Computation time for training optimization: {t_training_optimization.elapsed:.4f} sec")
                ...
                if (steps_til_checkpoint is not None) and (train_steps_current % steps_til_checkpoint == 0):
                    save_models_to_disk([(model, 'model'), (ema_model, 'ema_model')],
                                        epoch, train_steps_current, ckpt_dir)
                    save_losses_to_disk(train_losses_l, validation_losses_l, ckpt_dir)
