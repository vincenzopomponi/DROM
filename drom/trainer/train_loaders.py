...
from drom import models, losses, datasets, summaries
from drom.utils import model_loader, pretrain_helper
...
def get_dataset(dataset_class=None,
    ...
    full_dataset = DatasetClass(tasks=tasks, number_of_trajs=number_of_trajs, horizon=horizon, normalizer=normalizer,
                                context=context, dataset_dir=dataset_dir, tensor_args=tensor_args)
    print(f"\nDataset:\n{full_dataset}")

    # split into train and validation
    proportions = [1-val_set_size, val_set_size]
    lengths = [int(p * len(full_dataset)) for p in proportions]
    if not sum(lengths) == len(full_dataset):
        lengths[0] += abs(len(full_dataset) - sum(lengths))
    train_subset, val_subset = random_split(full_dataset, lengths=lengths)
    train_dataloader = DataLoader(train_subset, batch_size=batch_size)
    val_dataloader = DataLoader(val_subset, batch_size=batch_size)
    print(f"\nLength training subset: {len(train_subset)}")
    print(f"Length validation subset: {len(val_subset)}")
