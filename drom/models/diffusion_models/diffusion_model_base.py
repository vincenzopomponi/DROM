...
from drom.models.diffusion_models.helpers import cosine_beta_schedule, Losses, exponential_beta_schedule
from drom.models.diffusion_models.sample_functions import extract, apply_hard_conditioning, guide_gradient_steps, \
    ddpm_sample_fn
from drom.torch_utils.torch_timer import TimerCUDA
from drom.torch_utils.torch_utils import to_numpy
...
def build_context(model, dataset, input_dict):
    ...
    if model.context_model is not None:
        context = dict()
        task_normalized = input_dict[f'{dataset.field_key_hard_conds}_normalized']
        task_normalized = task_normalized[:, 2*dataset.trajectory_dim[1]:]
...
class GaussianDiffusionModel(nn.Module, ABC):
    ...
    def warmup(self, horizon=64, device='cuda'):
        print("")
        for _ in tqdm(range(100), desc="Warming up the model...", unit="iter"):
            shape = (2, horizon, self.state_dim)
            x = torch.randn(shape, device=device)
            t = make_timesteps(2, 1, device)
            if self.context_model is not None:
                context = torch.randn((shape[0], self.context_model.out_dim), device=device)
            else:
                context = None
    ...
    def p_losses(self, x_start, context, t, hard_conds):
        ...
        # MPD original noise scheduler version
        x_noisy = self.q_sample(x_start=x_start, t=t, noise=noise)

        # DDPM schedulers
        # pos = self.position_noise_scheduler.add_noise(x_start[..., :3], noise[..., :3], t)
        # rot = self.rotation_noise_scheduler.add_noise(x_start[..., 3:6], noise[..., 3:6], t)
        # x_noisy = torch.cat((pos, rot), -1)
        ...
        # context model
        if context is not None:
            # print(f"context in: {context['tasks'].size()}")
            context = self.context_model(context)
            # input(f"context out: {context.size()}")
    ...
    def loss(self, x, context, *args):
        # MPD original version
        batch_size = x.shape[0]
        t = torch.randint(0, self.n_diffusion_steps, (batch_size,), device=x.device).long()

        # DDPM version
        # t = torch.randint(0, self.position_noise_scheduler.config.num_train_timesteps, (len(x),), device=x.device).long()
