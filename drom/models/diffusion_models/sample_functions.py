...
def apply_hard_conditioning(x, conditions):
    """
    Setting start and final position of the trajectory equal to start and goal desired position.
    Practically:
    - \tau[0] = s_s
    - \tau[H-1] = s_g
    """
    # print("Appllying hard conditions...")

    for t, val in conditions.items():
        # print(f"t: {t}")
        # print(f"val: {val.size()}")
        x[:, t, :] = val.clone()
    # input("Hard consitions applied!")
    return x
